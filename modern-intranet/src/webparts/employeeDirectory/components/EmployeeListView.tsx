...
const EmployeeRow: React.FC<{ employee: IEmployee, onClick: () => void }> = ({ employee, onClick }) => {
    const renderAvatar = (size: number): JSX.Element => {
        ...
        const colors = ['#0078d4', '#038387', '#8764b8', '#c239b3', '#ca5010', '#498205', '#005b70', '#7160e8'];
        const codeValue = employee.name.codePointAt(0);
        const color = colors[(codeValue || 0) % colors.length];

        return (
            <div className={styles.avatarBase} style={{ background: color, width: size, height: size, fontSize: Math.round(size * 0.35) }}>
                {initials}
            </div>
        );
    };
    ...
    return (
        ...
        >
            ...
            <div className={styles.info}>
                ...
                <div className={styles.role}>{employee.jobTitle} &middot; {employee.department}</div>
...
const EmployeeListView: React.FC<IEmployeeListViewProps> = ({ employees, onEmployeeClick }) => {
    return (
        <div className={styles.listView}>
            {employees.map(emp => (
                <EmployeeRow key={emp.id} employee={emp} onClick={() => onEmployeeClick(emp)} />
            ))}
        </div>
    );
};
